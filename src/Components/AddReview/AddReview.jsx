...
const AddReview = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch("http://localhost:3000/pruducts", {
        method: "POST",
    ...
    <div className="max-w-xl mx-auto p-6 mt-10 bg-white shadow rounded">
      ...
      <form onSubmit={handleSubmit} className="space-y-3">
        ...
        <textarea
          ...
          required
          className="w-full px-3 border rounded h-24"
        ></textarea>

        <button
          type="submit"
          className="w-full h-12 bg-orange-500 text-white rounded hover:bg-orange-600"
        >
          Add Review
        </button>
      </form>
