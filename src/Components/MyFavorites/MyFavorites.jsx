...
const MyFavorites = () => {
  ...
  const handleRemove = async (id) => {
    try {
      const res = await fetch(`https://assignment10-server-ruby-beta.vercel.app/favorites/${id}`, {
        method: "DELETE",
      });
      const data = await res.json();
  ...
  <section className="max-w-6xl mx-auto my-10 px-4">
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {favorites.map((fav) => (
        <div key={fav._id} className="bg-white shadow rounded p-4 flex flex-col">
          <img
            src={fav.foodImage}
            alt={fav.foodName}
          ...
          {/* Remove Button under the picture */}
          <button
            onClick={() => handleRemove(fav._id)}
            className="mb-4 px-3 py-1 bg-red-500 text-white rounded hover:bg-red-600 self-start"
